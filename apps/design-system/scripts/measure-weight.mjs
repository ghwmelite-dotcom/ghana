#!/usr/bin/env node
/**
 * Measure gzipped weight of `/`. The design-system site is a docs
 * surface not a citizen-facing service; we hold it to the same 80 KB
 * budget the portal homepage uses, because if the design docs cannot
 * live within their own principles the principles are not credible.
 */
...
async function main() {
  ...
  const dom = new JSDOM(html, { url: 'https://design.gov.gh/' });
  ...
  for (const r of rows) {
    console.log(
      '  ' + r.name.padEnd(46) + String(r.raw).padStart(10) + String(r.gzip).padStart(10),
    );
  }
  console.log('  '.padEnd(48) + '----'.padStart(10) + '----'.padStart(10));
  console.log(
