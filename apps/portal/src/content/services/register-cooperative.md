---
slug: register-cooperative
title: Register a cooperative society
lifeStage: business
topics: [cooperative, business-registration, agriculture]
lede: >-
  Register a cooperative society with the Department of Cooperatives under
  the Cooperative Societies Decree, 1968 (NLCD 252). Registration gives your
  group legal status to own property, enter contracts, and access credit.
priority: 5
eligibility:
  - text: >-
      You are a group of at least 10 persons (or 3 cooperatives for a
      secondary cooperative) forming a society for a common economic purpose.
  - text: >-
      All founding members must be at least 18 years old and agree on a
      set of model by-laws covering the society's objectives, membership
      rules, and governance.
youWillNeed:
  - List of at least 10 founding members with their Ghana Cards
  - Draft by-laws of the cooperative (the Department can provide a model)
  - Minutes of the inaugural meeting signed by all founding members
  - Statement of common economic purpose (e.g., farming, savings, artisan trade)
  - Proposed society name
cost:
  amount: 50
  currency: GHS
  unit: registration fee
  notes: >-
    Annual filing fees apply after registration. The Department of Cooperatives
    may charge inspection or supervision fees. Contact the regional office for
    the current schedule.
timeline: >-
  Registration is typically completed within 10 working days after all documents
  are reviewed and approved.
agency:
  name: Department of Cooperatives
  url: https://mofep.gov.gh/cooperatives
  phone: +233 30 251 9800
applyUrl: https://mofep.gov.gh/cooperatives/register
updatedAt: 2026-04-19
sourceOfTruth: https://mofep.gov.gh
---

## Before you apply

Hold an inaugural meeting of all founding members, elect provisional officers,
and agree on the by-laws. The Department of Cooperatives can provide a model
by-laws template to speed up the process.

## Step by step

1. Hold the inaugural meeting and prepare the minutes, signed by all members.
2. Draft your by-laws or adapt the Department's model template.
3. Collect Ghana Cards or identity details of all founding members.
4. Submit the application form, minutes, by-laws, and member list to the
   District Department of Cooperatives office.
5. Pay the registration fee.
6. The Department reviews, registers the society, and issues a certificate
   of registration.

## After you register

Open a bank account in the cooperative's name using the certificate. File
annual returns with the Department. The Department may conduct supervisory
inspections to ensure the cooperative operates within its by-laws.
